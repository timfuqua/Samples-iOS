...
// *********** End of Ray Wenderlich examples ***********

func test20() {
  
  let allStrings = ["A", "B", "A10", "A20", "A1", "A2"]
  println(sorted(allStrings))
  
  var stringsWithNumericSuffix: [String] = []
  
  for value in allStrings {
    var matches = listMatches("\\w+[0-9]{1,2}$", inString: value)
    if matches.count > 0 {
      stringsWithNumericSuffix += matches
    }
  }
  
  println(sorted(stringsWithNumericSuffix))
}

func compareStringsWithNumericSuffix(lhs: String, rhs: String) -> Bool {
  
  var lhsParts = listGroups("^(.*?)\\s*(_?\\d+)$", inString: lhs)
  var rhsParts = listGroups("^(.*?)\\s*(_?\\d+)$", inString: rhs)
  
  if lhsParts.count == 3 && rhsParts.count == 3 {
    if lhsParts[1] == rhsParts[1] {
      if lhsParts[2].toInt() != nil && rhsParts[2].toInt() != nil {
        return lhsParts[2].toInt()! < rhsParts[2].toInt()!
      }
    }
  }
...
func test21() {
  ...
  var lhsNumericSuffix = listMatches("\\w+[0-9]{1,2}$", inString: lhs)
  var rhsNumericSuffix = listMatches("\\w+[0-9]{1,2}$", inString: rhs)
  
  if lhsNumericSuffix.count > 0 {
    println("lhs: \(lhs) has a numeric suffix")
  }
  
  if rhsNumericSuffix.count > 0 {
    println("rhs: \(rhs) has a numeric suffix")
  }
}
...
//  let lhs = "10"
  ...
  if lhsParts.count == 3 {
    println("lhs prefix: \(lhsParts[1])  lhs suffix: \(lhsParts[2])")
  }
...
func test23() {
  ...
  println("Should \(lhs) come before \(rhs)? \(compareStringsWithNumericSuffix(lhs, rhs))")
}

func test24() {
  
  let codes = ["ACDIN", "DACWOUT", "AUX10", "AUX1", "AUX11", "AUX20", "AUX2"]

  println("Unsorted: \(codes)")
  println("Sorted: \(sorted(codes, compareStringsWithNumericSuffix))")
}


// call the tests you want to run here
